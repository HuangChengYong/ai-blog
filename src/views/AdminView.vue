...
// --- 引入拆分后的 Admin 接口 (新增了增删改查 API) ---
import {
  loadAdminMenus,
  loadAdminOverview,
  loadAdminPermissionGroups,
  loadAdminRoles, getAdminRole, createAdminRole, updateAdminRole,
  loadAdminUsers, createAdminUser, updateAdminUser, deleteAdminUser,
  type AdminMenu, type AdminOverview, type AdminPermissionGroup, type AdminRole,
  type AdminStudioOptions, type AdminUser, type UpdateRoleRequest, type UpdateUserRequest, loadAdminStudioOptions
} from '../services/admin'
...
const userForm = reactive<UpdateUserRequest>({ username: '', password: '', roleId: '', dataScope: 'SELF' })
const roleForm = reactive<UpdateRoleRequest>({ name: '', description: '', permissionCodes: [] })
...
const openUserEdit = (user?: AdminUser) => {
  ...
  if (user) {
    ...
    Object.assign(userForm, { username: user.name, roleId: user.role, dataScope: user.scope })
  } else {
    Object.assign(userForm, { username: '', password: '', roleId: '', dataScope: 'SELF' })
  }
...
const handleSaveUser = async () => {
  try {
    isEdit.value
        ? await updateAdminUser(currentId.value, userForm)
...
// --- 新增：角色管理弹窗交互 ---
const openRoleEdit = async (role?: AdminRole) => {
  isEdit.value = !!role
  if (role) {
    currentId.value = role.id
    // 调用接口获取角色的详细权限码
    const detail = await getAdminRole(role.id)
    Object.assign(roleForm, { name: detail.name, description: detail.description, permissionCodes: detail.permissionCodes || [] })
  } else {
    Object.assign(roleForm, { name: '', description: '', permissionCodes: [] })
  }
...
const handleSaveRole = async () => {
  try {
    if (isEdit.value) {
      await updateAdminRole(currentId.value, roleForm)
    } else {
      await createAdminRole({
        ...roleForm,
        roleCode: roleForm.name.trim().toUpperCase().replace(/\s+/g, '_'),
      })
    }
    ElMessage.success('操作成功')
    roleDialogVisible.value = false
    await loadSectionData('roles')
  } catch (e: any) {
    ElMessage.error(e.message || '操作失败')
  }
}
...
<template>
  <section class="admin-page">
        ...
        <main class="admin-content">
          ...
          <section v-if="activeSection === 'users'" class="admin-page-section">
            <div class="admin-page-head">
              ...
              <el-button type="primary" :icon="User" @click="openUserEdit()">新建用户</el-button>
            </div>
            <section class="admin-table-card">
              <article v-for="user in users" :key="user.id" class="user-row" style="display: flex; align-items: center; padding: 15px; border-bottom: 1px solid #f0f0f0;">
                <span class="admin-avatar"><el-icon><UserFilled /></el-icon></span>
                <div style="flex: 1;">
                  <h3>{{ user.name }}</h3>
                  <p>{{ user.scope }}</p>
                ...
                <el-tag :type="user.status === '在线' || user.status === '启用' ? 'success' : 'info'" effect="plain" style="margin-left: 10px;">{{ user.status }}</el-tag>
                <span style="margin: 0 15px;">{{ user.lastSeen }}</span>

                <div class="draft-action-bar" style="margin-left: 20px;">
                  <el-button :icon="EditPen" @click="openUserEdit(user)">编辑</el-button>
                  <el-button type="danger" :icon="DeleteIcon" @click="handleDeleteUser(user.id)">删除</el-button>
                </div>
              </article>
            </section>
          </section>
          ...
          <section v-if="activeSection === 'roles'" class="admin-page-section">
            <div class="admin-page-head">
              ...
              <el-button type="primary" :icon="Key" @click="openRoleEdit()">新建角色</el-button>
            </div>
              ...
              <article v-for="role in roles" :key="role.name" class="role-card">
                <div class="section-heading">
                  ...
                  <div style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <el-button link :icon="EditPen" @click="openRoleEdit(role)" />
                  </div>
                </div>
    ...
    <el-dialog v-model="roleDialogVisible" :title="isEdit ? '编辑角色' : '新建角色'" width="600px">
      <el-form label-position="top">
        ...
        <el-form-item label="功能权限">
          <section class="permission-editor" style="max-height: 300px; overflow-y: auto; border: 1px solid #dcdfe6; padding: 15px; border-radius: 4px;">
            <div v-for="group in permissionGroups" :key="group.title">
              <h4 style="margin: 10px 0; color: #606266; font-size: 14px;">{{ group.title }}</h4>
              <el-checkbox-group v-model="roleForm.permissionCodes">
                <el-checkbox v-for="p in group.items" :key="p.code" :value="p.code" :label="p.code">
                  {{ p.name }}
                </el-checkbox>
              </el-checkbox-group>
            </div>
